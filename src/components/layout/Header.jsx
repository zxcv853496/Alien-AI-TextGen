            ...
            <div className="flex items-center gap-4 md:gap-6">
                ...
                <div className="flex items-center text-sm font-medium text-slate-600 bg-slate-100 rounded-lg p-1">
                    <button
                        ...
                        className={cn("px-3 py-1 rounded-md transition-all", i18n.language === 'zh' ? 'bg-white text-indigo-600 shadow-sm' : 'hover:text-slate-900')}
                    >
                        繁體
                    </button>
                    <button
                        onClick={() => changeLanguage('en')}
                        className={cn("px-3 py-1 rounded-md transition-all", i18n.language === 'en' ? 'bg-white text-indigo-600 shadow-sm' : 'hover:text-slate-900')}
                    >
                        English
                    </button>
                </div>
                ...
                {!isLoggedIn ? (
                    <>
                        ...
                        <button onClick={onLogin} className="text-sm font-bold text-indigo-600 hover:text-indigo-700 px-4 py-2 hover:bg-indigo-50 rounded-lg transition-colors">{t('nav.login')}</button>
                        <button onClick={onRegister} className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-md">{t('nav.register')}</button>
                    </>
                ) : (
                    <>
